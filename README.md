...
- 来場者向け：日付切替、企画検索、カテゴリ絞込、お気に入り、待ち時間、情報鮮度、タイムテーブル、開催中／次の演目表示
- スタッフ向け：PINログイン、列人数更新、案内完了、営業状態、売切・受付終了、お知らせ更新
- データ管理：企画・団体一覧とタイムテーブルのCSV/JSON取込、テンプレート出力、取込前検証、追加・更新／確認付き全件置換、自動・手動JSONバックアップ
- 全体告知：中止・会場変更・入場制限を全来場者へ即時表示し、解除まで一元管理
- 障害対策：10秒ポーリング、端末キャッシュ、通信断時の閲覧、更新保留、復旧後再送、リビジョンによる競合検知、古い待ち時間の非表示
- 配布：レスポンシブUI、PWA、GitHub Pages向けワークフロー、Supabase Edge Function
...
環境変数が空の場合はデモモードで動作します。デモモードの初期PINは `202608` で、データはその端末のブラウザ内だけに保存されます。複数端末で運用する本番では、必ず共有APIを設定してください。

## 本番用バックエンド（Supabase）

1. Supabaseプロジェクトを作成します。
2. SQL Editorで `supabase/migrations/001_init.sql` を実行します。
...
- 初期PIN `202608` は本番前に必ず変更してください。
- PINをQRコード、公開資料、来場者向けサイトへ掲載しないでください。
- SupabaseのサービスロールキーをGitHub、フロント環境変数、スクリーンショットへ載せないでください。
- 本番前に全件バックアップを保存してください。全件置換時はアプリも反映直前に自動バックアップを出力します。
